...
async def process_prompt(request: Prompt):
    ...
    try:
        ...
        log_message_csv = [
            timestamp, embed_model, prompt, best_route.name, similarity, 'euclidean', prompt_embedding,
...
def find_best_route(prompt_embedding, routes):
    best_route = None
    best_similarity = float('inf')  # Initialize to a large value since we seek the minimum distance

    for route in routes:
        for utterance in route.utterances:
            utterance_embedding, _ = get_embedding(utterance)
            similarity = euclidean_distance(prompt_embedding, utterance_embedding)
            if similarity < best_similarity:  # Smaller distance is more similar
